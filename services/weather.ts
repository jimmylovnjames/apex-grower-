...
export const VPD_TARGETS: Record<GrowStage, { min: number; max: number; label: string }> = {
  ...
  [GrowStage.MEDIUM_PREP]: { min: 0.4, max: 1.2, label: 'n/a — target 18–24°C in the medium' },
...
export const vpdVerdict = (vpd: number, stage: GrowStage): string => {
  ...
  if (stage === GrowStage.MEDIUM_PREP) return 'Not applicable before planting — watch medium temperature instead';
